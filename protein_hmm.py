...
def main(models_dir, fastas_dir, output_dir):
    ...
    # Verificar e criar o diretÃƒÂ³rio de saÃƒÂ­da, se necessÃƒÂ¡rio
    ...
    for model in models:
        ...
        for fasta in fastas:
            # Extrair o nÃƒÂºmero de acesso do nome do arquivo FASTA
            accession = os.path.basename(fasta).split(".")[0]

            # Definir o nome do arquivo de saÃƒÂ­da com base no modelo e no arquivo FASTA
        ...
        # ApÃƒÂ³s o loop de fasta, mas ainda dentro do loop de modelo:
        if all_results:
    ...
    print("Busca e conversÃƒÂ£o concluÃƒÂ­das!")
...
def read_hmmer(path, program="hmmsearch", format="tblout", add_header_as_index=False, verbose=True):
   ...
   # Criar um dataframe vazio para 'tblout'
    ...
    # Extrair a espÃƒÂ©cie de cada linha da coluna "query_description" usando expressÃƒÂµes regulares
    df['Specie'] = df[('identifier', 'query_description')].str.extract(r'\[(.*?)\]')

    # Verifique se a extraÃƒÂ§ÃƒÂ£o foi bem-sucedida e preencha os valores ausentes
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Execute hmmsearch em arquivos .faa usando modelos do diretÃƒÂ³rio fornecido.")
    parser.add_argument("models_dir", help="DiretÃƒÂ³rio contendo os modelos HMM (.hmm).")
    parser.add_argument("fastas_dir", help="DiretÃƒÂ³rio contendo os arquivos FASTA (.faa).")
    parser.add_argument("output_dir", help="DiretÃƒÂ³rio para salvar os resultados.")
