...
def extract_genome_id(subdir_name):
    """
    Extrai o ID do genoma do nome do subdiretório usando expressões regulares.
    O ID pode ter diferentes prefixos como 'GCA_', 'GCF_', 'BGC', 'WP_', etc.
    ...
    """
    # Expressão regular para capturar diferentes padrões de IDs
    pattern = r'((?:GCA|GCF|WP)_[\d.]+|BGC\d+)'
    match = re.search(pattern, subdir_name)
    ...
    with open(log_file, "w") as log:
        for subdir in input_dir.iterdir():
            if subdir.is_dir() and subdir.name.startswith("Result_"):
                ...
                log.write(f"Processando subdiretório: {subdir.name} (Genome ID: {genome_id})\n")
                gbk_files = list(subdir.glob("*.gbk"))
                if len(gbk_files) == 0:
                    log.write(f"  Ignorado: Nenhum arquivo .gbk encontrado\n")
                ...
                for gbk_file in gbk_files:
                    try:
                        ...
                        log.write(f"  Analisando arquivo: {gbk_file.name}\n")
                        ...
                        if file_has_cal:
                            ...
                            file_size_mb = file_size_bp / 1_000_000  # Convertendo para megabases
                            subdir_size_mb += file_size_mb
                            files_with_cal += 1
                            size_mb_str, size_gb_str = format_size(file_size_mb)
                            log.write(f"    Encontrado 'CAL_domain' x{file_cal_count} (Tamanho: {size_mb_str} / {size_gb_str})\n")

                            # Adiciona o arquivo à lista de arquivos a serem copiados
                            gbk_files_to_copy.append(gbk_file)
                        else:
                            log.write(f"    'CAL_domain' não encontrado neste arquivo.\n")
                    except Exception as e:
                ...
                if cal_count > 0:
                    ...
                    for gbk_file in gbk_files_to_copy:
                        try:
                            ...
                            log.write(f"    Copiado para: {destination_file}\n")
                        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Analisa subdiretórios com domínios CAL_domain e opcionalmente AMP-binding, calculando o tamanho total dos arquivos .gbk encontrados para verificar espaço em disco disponível. Além disso, copia os arquivos identificados para um diretório filtrado com prefixo Genome ID."
    )
    parser.add_argument(
        "input_dir", type=str, help="Diretório de entrada contendo subdiretórios com arquivos .gbk."
    )
    parser.add_argument(
        "log_file", type=str, help="Arquivo de log para salvar informações sobre o processamento."
    )
    parser.add_argument(
        "--search-amp-binding",
        action="store_true",
